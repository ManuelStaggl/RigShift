...
/// profile before 1.9 means), switch it off, or run this grid. There is no grid editor: a grid is built once in the NVIDIA
...
public sealed partial class SurroundSection : ObservableObject
{
    ...
    public SurroundSection(SurroundState state, SurroundSetting? saved)
    {
        ...
        if (IsVisible)
        {
            FillChoices(saved is null ? Unchanged : saved.Enabled ? On : Off);
        }
    }
