...
/// </summary>
public sealed partial class AutomationViewModel : ObservableObject, IRuleOwner
{
    ...
    private void Rebuild(IReadOnlyList<AutomationRule> rules) => Quietly(() =>
    {
        ...
        List<Guid> missing = rules.SelectMany(r => new[] { r.ProfileId, r.ExitProfileId ?? r.ProfileId }).Distinct().Where(id => _catalog.Find(id) is null).ToList();
        foreach (Guid id in missing)
        {
            ProfileChoices.Add(new Choice(id.ToString("D"), Loc.Instance["Automation_MissingProfile"]));
        }

        RuleExits.Fill(ExitChoices, _catalog.Profiles, missing);
    ...
    public Choice? ProfileChoiceFor(Guid id) => ProfileChoices.FirstOrDefault(c => c.Key == id.ToString("D"));
    ...
    private async Task AddRuleAsync()
    {
        ...
        (Guid profile, Guid exitProfile) = RuleExits.NewRuleProfiles(_catalog.Profiles, _settings.Current.DefaultProfileId);
